...
import dev.hardwood.internal.iotrace.CaptureRecords;
...
/// [RecordSet] that [dev.hardwood.internal.iotrace.PlanExtractor] validates.
///
    ...
    public static RecordSet read(Path path) throws IOException {
        ...
        try (RecordingFile file = new RecordingFile(path)) {
            while (file.hasMoreEvents()) {
                ...
                switch (name) {
                    case "dev.hardwood.iotrace.PlanNode" -> planNodes.add(new CaptureRecords.PlanNode(
                    ...
                    case "dev.hardwood.iotrace.PlanEdge" -> planEdges.add(new CaptureRecords.PlanEdge(
                            event.getLong("executionId"), event.getLong("planId"),
                            event.getLong("fromNodeId"), event.getLong("toNodeId"),
                            event.getString("edgeKind")));
                    case "dev.hardwood.iotrace.PlanSealed" -> planSeals.add(new CaptureRecords.PlanSealed(
                            event.getLong("executionId"), event.getLong("planId"),
                    ...
                    case "dev.hardwood.iotrace.Request" -> requests.add(new CaptureRecords.Request(
                            event.getLong("executionId"), event.getLong("planId"), event.getLong("nodeId"),
